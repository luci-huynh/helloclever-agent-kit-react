...
7. **No AI attribution in projects** (core rule R6.6, in section 6 so it cannot be overridden): nothing pushed or posted from a project may credit an AI tool. Enforced by `agent-kit init` through `.claude/settings.json` (`attribution` off) and a `commit-msg` git hook (`workflows/hooks/commit-msg.js`). The hook matches attribution patterns, not bare tool names, so legitimate mentions (`CLAUDE.md`, a feature integrating an AI API) pass.
